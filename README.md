...
UI の `Share URL` / `Share ID` で、ノードグラフ(JSON)を共有できます。

- `Share URL`  
  グラフJSONを圧縮（LZString）してURLハッシュに埋め込み、クリップボードにコピーします。
- `Share ID`  
  グラフJSONを外部ストレージ（`tmpfiles.org`）へ保存し、短いID付きURLをクリップボードにコピーします。
- 受信側はURLを開くだけで自動読込されます（`g` または `sid` を検出）。
...
## ライセンス
このプロジェクト本体のコードは Apache License 2.0 です。  
サードパーティライブラリ／フォントのライセンス情報は `THIRD_PARTY_NOTICES.md` を参照してください。
